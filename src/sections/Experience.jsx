const experiences = [
  {
    period: "2025",
    role: "Production Systems Automation Intern",
    company: "Engtek Precision Philippines, Inc.",
    description:
      "Contributed the development of Excel-based automation tools using VBA for tracking, inventory monitoring, and reporting.",
    technologies: ["Microsoft Suite", "VBA"],
    current: false,
  },
  {
    period: "2022 — 2025",
    role: "Freelance Graphic Designer",
    company: "Self-Employed",
    description:
      "Delivered custom digital character illustrations, logo designs, and social media info-cards. Discussed project goals, themes, and styles with clients",
    technologies: [
      "Photoshop",
      "AfterEffects",
      "Illustrator",
      "ClipStudio",
      "Live2d",
    ],
    current: false,
  },

  {
    period: "2021 — 2025",
    role: "BS Computer Engineering",
    company: "National University - Laguna",
    description: "College",
    technologies: [
      "Arduino",
      "Python",
      "JavaScript",
      "HTML",
      "CSS",
      "Firebase",
    ],
    current: false,
  },
  {
    period: "2019 — 2021",
    role: "STEM",
    company: "Laguna College of Business and Arts",
    description: "Senior year",
    technologies: ["Science", "Technology", "Engineering", "Mathematics"],
    current: false,
  },
];

export const Experience = () => {
  return (
    <section id="experience" className="py-32 relative overflow-hidden">
      {/* BACKGROUND */}
      <div className="absolute top-20 left-25 w-96 h-96 bg-primary/5 rounded-full blur-3xl" />
      <div className="container mx-auto px-6 relative z-10">
        {/* HEADER TXT*/}
        <div className="max-w-3xl mb-16">
          <h1
            className="text-secondary-foreground text-sm font-medium tracking-wider uppercase 
            animate-fade-in"
          >
            Path & Progress
          </h1>

          <h2
            className="text-4xl md:text-5xl font-bold
           mt-4 mb-6 animate-fade-in animate-delay-100 text-secondary-foreground"
          >
            Academic Foundations to{" "}
            <span className="font-serif italic font-normal text-white">
              Practical Experience.
            </span>
          </h2>

          <p className="text-muted-foreground animate-fade-in animate-delay-200">
            A timeline of my growth through education, hands-on experience, and
            continuous learning.
          </p>
        </div>

        {/* TIME */}
        <div className="relative">
          {/* ITEMS 1 */}
          <div
            className="absolute md:left-1/2 top-0 bottom-0 w-0.5 bg-linear-to-b
           from-primary/70 via-primary/30 to-transparent animate-fade-in"
          />

          <div className="space-y-12">
            {experiences.map((exp, idx) => (
              <div
                key={idx}
                className="relative grid md:grid-cols-2 gap-8 animate-fade-in"
                style={{ animationDelay: `${(idx + 1) * 150}ms` }}
              >
                <div className="absolute left-0 md:left-1/2 top-0 w-3 h-3 bg-primary rounded-full -translate-x-1/2 ring-4 ring-background z-10">
                  {exp.current && (
                    <span className="absolute inset-0 rounded-full bg-primary animate-ping opacity-75" />
                  )}
                </div>

                <div
                  className={`pl-8 md:pl-0 ${
                    idx % 2 === 0
                      ? "md:pr-16 md:text-right"
                      : "md:col-start-2 md:pl-16"
                  }`}
                >
                  <div
                    className={`glass p-6 rounded-2xl border border-primary/30 hover:border-primary/50 transition-all duration-500`}
                  >
                    <span className="text-sm text-primary font-medium">
                      {exp.period}
                    </span>
                    <h3 className="text-xl font-semibold mt-2">{exp.role}</h3>
                    <p className="text-muted-foreground">{exp.company}</p>
                    <p className="text-sm text-muted-foreground mt-4">
                      {exp.description}
                    </p>

                    <div
                      className={`flex flex-wrap gap-2 mt-4 ${idx % 2 === 0 ? "md:justify-end" : ""}`}
                    >
                      {exp.technologies.map((tech, techIdx) => (
                        <span
                          key={techIdx}
                          className="px-3 py-1 bg-surface text-xs rounded-full text-muted-foreground"
                        >
                          {tech}
                        </span>
                      ))}
                    </div>
                  </div>
                </div>
              </div>
            ))}
          </div>

          {/* ITEMS 2 */}
          {/* <div className="gap-8 grid grid-cols-2">
            {experiences.map((exp, idx) => (
              <div
                key={idx}
                className="relative animate-fade-in"
                style={{ animationDelay: `${(idx + 1) * 150}ms` }}
              >
               
                <div className="absolute left-0 md:left-1 top-0 w-3 h-3 bg-primary rounded-full -translate-x-1/2 ring-4 ring-background z-10">
                  {exp.current && (
                    <span className="absolute inset-0 rounded-full bg-primary animate-ping opacity-75" />
                  )}
                </div>

                <div className=" ">
                  <div
                    className={`glass p-6 rounded-2xl border border-primary/30 hover:border-primary/50 transition-all duration-500`}
                  >
                    <span className="text-sm text-primary font-medium">
                      {exp.period}
                    </span>
                    <h3 className="text-xl font-semibold mt-2">{exp.role}</h3>
                    <p className="text-muted-foreground">{exp.company}</p>
                    <p className="text-sm text-muted-foreground mt-4">
                      {exp.description}
                    </p>

                    <div
                      className={`flex flex-wrap gap-2 mt-4 ${idx % 2 === 0 ? "md:justify-end" : ""}`}
                    >
                      {exp.technologies.map((tech, techIdx) => (
                        <span
                          key={techIdx}
                          className="px-3 py-1 bg-surface text-xs rounded-full text-muted-foreground"
                        >
                          {tech}
                        </span>
                      ))}
                    </div>
                  </div>
                </div>
              </div>
            ))}
          </div> */}
        </div>
      </div>
    </section>
  );
};
